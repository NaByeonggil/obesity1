import { PrismaClient } from '@prisma/client'

const prisma = new PrismaClient()

async function main() {
  console.log('Adding clinic dummy data...')

  // 기존 의원 사용자들 조회
  const existingDoctors = await prisma.user.findMany({
    where: { role: 'DOCTOR' }
  })

  if (existingDoctors.length === 0) {
    console.log('No existing doctors found. Creating clinic dummy users and data...')

    // 의원 더미 사용자 데이터 생성
    const clinicData = [
      {
        email: 'seoul.family.clinic@example.com',
        name: '서울 가정의학과의원',
        role: 'DOCTOR' as const,
        phoneNumber: '02-1234-5678',
        address: '서울특별시 강남구 테헤란로 123',
        specialization: '가정의학과',
        experience: 15,
        rating: 4.8,
        coordinates: { lat: 37.5016, lng: 127.0375 }
      },
      {
        email: 'busan.internal.clinic@example.com',
        name: '부산 내과클리닉',
        role: 'DOCTOR' as const,
        phoneNumber: '051-2345-6789',
        address: '부산광역시 해운대구 센텀로 456',
        specialization: '내과',
        experience: 12,
        rating: 4.6,
        coordinates: { lat: 35.1687, lng: 129.1313 }
      },
      {
        email: 'daegu.ortho.clinic@example.com',
        name: '대구 정형외과의원',
        role: 'DOCTOR' as const,
        phoneNumber: '053-3456-7890',
        address: '대구광역시 중구 중앙대로 789',
        specialization: '정형외과',
        experience: 20,
        rating: 4.9,
        coordinates: { lat: 35.8714, lng: 128.6014 }
      },
      {
        email: 'incheon.pediatric.clinic@example.com',
        name: '인천 소아과클리닉',
        role: 'DOCTOR' as const,
        phoneNumber: '032-4567-8901',
        address: '인천광역시 연수구 컨벤시아대로 321',
        specialization: '소아과',
        experience: 8,
        rating: 4.7,
        coordinates: { lat: 37.3939, lng: 126.6369 }
      },
      {
        email: 'gwangju.derma.clinic@example.com',
        name: '광주 피부과의원',
        role: 'DOCTOR' as const,
        phoneNumber: '062-5678-9012',
        address: '광주광역시 서구 상무대로 654',
        specialization: '피부과',
        experience: 10,
        rating: 4.5,
        coordinates: { lat: 35.1595, lng: 126.8526 }
      }
    ]

    // 의원 사용자 생성
    const createdDoctors = []
    for (const clinic of clinicData) {
      const { coordinates, ...userData } = clinic
      const doctor = await prisma.user.create({
        data: userData
      })
      createdDoctors.push({ ...doctor, coordinates })
      console.log(`Created doctor: ${doctor.name}`)
    }

    // 진료과목 조회 또는 생성
    const departments = ['가정의학과', '내과', '정형외과', '소아과', '피부과']
    const departmentRecords = []

    for (const deptName of departments) {
      let department = await prisma.department.findFirst({
        where: { name: deptName }
      })

      if (!department) {
        department = await prisma.department.create({
          data: {
            name: deptName,
            description: `${deptName} 전문 진료`,
            isOnlineAvailable: Math.random() > 0.3 // 70% 확률로 온라인 진료 가능
          }
        })
        console.log(`Created department: ${department.name}`)
      }
      departmentRecords.push(department)
    }

    // 각 의원의 진료과목 연결 및 진료비 설정
    for (let i = 0; i < createdDoctors.length; i++) {
      const doctor = createdDoctors[i]
      const department = departmentRecords[i]

      // 의원-진료과목 연결
      await prisma.userDepartment.create({
        data: {
          userId: doctor.id,
          departmentId: department.id
        }
      })

      // 오프라인 진료비 설정
      await prisma.clinicFee.create({
        data: {
          doctorId: doctor.id,
          departmentId: department.id,
          consultationType: 'OFFLINE',
          basePrice: 15000 + Math.floor(Math.random() * 20000), // 15,000 ~ 35,000원
          emergencyPrice: 25000 + Math.floor(Math.random() * 15000), // 25,000 ~ 40,000원
          followUpPrice: 8000 + Math.floor(Math.random() * 7000), // 8,000 ~ 15,000원
          isActive: true
        }
      })

      // 온라인 진료가 가능한 경우 온라인 진료비도 설정
      if (department.isOnlineAvailable) {
        await prisma.clinicFee.create({
          data: {
            doctorId: doctor.id,
            departmentId: department.id,
            consultationType: 'ONLINE',
            basePrice: 10000 + Math.floor(Math.random() * 15000), // 10,000 ~ 25,000원 (오프라인보다 저렴)
            emergencyPrice: 20000 + Math.floor(Math.random() * 10000), // 20,000 ~ 30,000원
            followUpPrice: 5000 + Math.floor(Math.random() * 5000), // 5,000 ~ 10,000원
            isActive: true
          }
        })
      }

      console.log(`Set up fees for ${doctor.name} - ${department.name}`)
    }

    console.log('Clinic dummy data created successfully!')
  } else {
    console.log('Doctors already exist. Skipping clinic creation.')
  }

  // 진료비 데이터 확인
  const fees = await prisma.clinicFee.findMany({
    include: {
      doctor: true,
      department: true
    }
  })

  console.log(`\nTotal clinic fees created: ${fees.length}`)
  fees.forEach(fee => {
    console.log(`${fee.doctor.name} - ${fee.department.name} (${fee.consultationType}): ${fee.basePrice.toLocaleString()}원`)
  })
}

main()
  .catch((e) => {
    console.error(e)
    process.exit(1)
  })
  .finally(async () => {
    await prisma.$disconnect()
  })